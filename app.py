...
# --- 4. GEMINI API CONFIGURATION (FIXED SCOPE) ---
@st.cache_resource
def configure_genai():
    # Fetch key from secrets
    api_key = st.secrets["GEMINI_API_KEY"]
    genai.configure(api_key=api_key)
    # Using 'gemini-1.5-flash' directly to avoid the 404 error
...
# Assign 'model' in the main scope so section 6 can see it
try:
...
if prompt := st.chat_input("Write a message..."):
    ...
    if model:
        ...
        ai_response = "AI Model not initialized. Please check configuration."
